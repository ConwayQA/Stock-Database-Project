...
public class ItemControllerTest {
	...
	public void readAllTest() {
		...
		items.add(new Item("Settlers of Catan", BigDecimal.valueOf(30.00), "resource management", 3L, 5L, 60L, 2L));
	...
	public void createTest() {
		...
		Item item = new Item(name, BigDecimal.valueOf(Double.parseDouble(price)), genre, Long.parseLong(minPlayers), 
								Long.parseLong(maxPlayers), Long.parseLong(avgPlayTime), 1L);
		Item savedItem = new Item(1L, "Carcassonne", BigDecimal.valueOf(20.00), "Tile based", 2L, 12L, 30L, 1L);
		Mockito.when(itemService.create(item)).thenReturn(savedItem);
		assertEquals(savedItem, itemController.create(1L));
	}
	...
	public void updateTest() {
		...
		Item item = new Item(Long.parseLong(id), name, BigDecimal.valueOf(Double.parseDouble(price)), genre, 
								Long.parseLong(minPlayers), Long.parseLong(maxPlayers), Long.parseLong(avgPlayTime), 1L);
		Mockito.when(itemService.update(item)).thenReturn(item);
		assertEquals(item, itemController.update(1L));
	}
